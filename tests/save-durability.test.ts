...
test('다트 팩 슬롯·morph 가 세이브 라운드트립에서 보존된다 (이번 작업 유실 방지)', () => {
  const withMag = {
    blasters: [
      {
        id: 'b_mag',
        ...
        createdAt: 7,
        parts: {
          body: { partId: 'body_bulldog', paint: {}, morph: {} },
          barrel: { partId: 'barrel_snap', paint: {}, morph: { barrelCount: 1 } },
          magazine: {
            partId: 'mag_drum',
            paint: { primary: { color: 'blasterLime', finish: 'gloss' } },
            morph: { magSize: 0.9, magLength: 0.2 },
          },
        },
      },
    ],
  }
  const s = normalizeSave(withMag, 9999)
  assert.equal(s.blasters.length, 1)
  const mag = s.blasters[0]!.parts.magazine!
  assert.equal(mag.partId, 'mag_drum', '다트 팩 슬롯이 사라짐')
  assert.equal(mag.morph.magSize, 0.9, 'magSize morph 유실')
  assert.equal(mag.morph.magLength, 0.2, 'magLength morph 유실')
  assert.equal(mag.paint.primary!.color, 'blasterLime', '다트 팩 색칠 유실')
})
