...
test('메시 수 예산(≤14) 준수 — 전 카탈로그 × 극단 morph(장식·미니건 포함)', () => {
  for (const id of M1_PARTS) {
    for (const morph of EXTREMES) {
      ...
      assert.ok(n <= PERFORMANCE_BUDGETS.maxPartMeshes, `${id} morph ${JSON.stringify(morph)} → ${n} meshes`)
      built.dispose()
    }
  }
})
...
test('8슬롯 완전 장착의 파츠별 최악 메시 합은 56 이하', () => {
  ...
  assert.ok(worst <= PERFORMANCE_BUDGETS.maxBlasterMeshes, `완전 장착 메시 상한 초과: ${worst}`)
})
...
test('알려진 슬롯 prefix라도 레시피 미등록 ID는 기본 파츠로 위장하지 않는다', () => {
  const built = buildPart('body_not_registered', { morph: {} })
  assert.equal(countMeshes(built.group), 1)
  built.dispose()
})
