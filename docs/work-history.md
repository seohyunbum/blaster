...
## 2026-07-25 — PVP 스테이지마다 다른 맵 + 보스 + 카메라·드론 개선 (사용자 요청)

10살 사용자 요청 "스테이지마다 다른 맵, 보스 추가(올라갈수록 더 센 보스), 화면 좀더 잘 돌아가게, 드론들 벽에 끼지 않게."
- **스테이지별 맵(arenaField STAGE_MAPS 5종)**: 스테이지마다 엄폐물 배치·바닥/벽/상자 색이 다른 5개 레이아웃. `setStage(n)` 이 맵을 다시 짓고 보스를 새로 소환.
- **보스 추가(Drone 클래스 boss 변형)**: 각 스테이지에 큰 드론 보스 1기(금색 왕관·큰 히트반경 1.15·HP `6+stage*3`으로 스테이지마다 강화). 보스 격파 시 `consumeBossDown()`→다음 스테이지. 상태바에 "👑 보스HP" 표시.
- **카메라 개선**: 포인터락 시야 감도 0.0024→0.0034, 피치 한계 1.2→1.25 — "화면 좀더 잘 돌아가게".
- **드론 벽 끼임 해결**: 적 이동에도 `resolveCircle`(엄폐물 원-밀어내기) 적용 + wander 목표를 엄폐물 안이면 재추첨. 벽에 박혀 멈추던 문제 해소.
- **스테이지 전환**: 팝 카운트 기반(POPS_PER_STAGE) 제거, 보스 격파 기반으로 변경. 5스테이지 보스까지 잡으면 클리어→만들기 이동+연승+1(기존 배선 유지).
- **검증**: 100테스트·typecheck·아키텍처·핫패스·build(번들 108/120KB)·브라우저 smoke(pvp 152콜/152메시/1.3ms) 로컬 통과. 버전 15→16.

## 2026-08-17 — 폰 홈화면 설치 앱(PWA) 지원 (사용자 요청)

10살 사용자 요청 "핸드폰에서 할 수 있는 앱으로 만들어줘."
- **PWA 최소 구성(서비스워커 없음)**: `public/manifest.webmanifest`(이름·아이콘·display:fullscreen·theme_color) + `index.html` 에 manifest 링크와 iOS 메타(apple-mobile-web-app-capable, apple-touch-icon 등) 추가. 홈화면에 추가하면 주소창 없이 전체화면 앱처럼 열림.
- **앱 아이콘 신규**: 게임 테마(주황 배경 + 드럼·파란 총열·노란 총구 토이 블래스터) PNG 4종 생성 — 192/512/512-maskable/apple-touch-icon(180). 08 안전 준수(실총 실루엣 아님, 토이성 우선).
- **서비스워커는 일부러 생략**: 오프라인 캐시가 "안 바뀌었어"(캐시) 혼란을 키울 수 있어 이번엔 설치성만. 오프라인 플레이는 추후 옵션.
- **주의(정직)**: PVP는 WASD+마우스룩(포인터락) 기반이라 터치폰에서는 조작이 어려움 — 터치 조이스틱/버튼은 다음 작업 후보.
- **검증**: verify(100테스트·아키텍처·핫패스)·build(번들 108/120KB, dist에 manifest·icons 복사 확인)·브라우저 smoke 통과. viewport 에 viewport-fit=cover 추가(노치 대응). 버전 16→17.

## 2026-08-17 — PVP 폰 터치 조종(조이스틱·쏘기 버튼) + 어깨끈 4종 추가 (사용자 요청)

10살 사용자 요청 "조이스틱과 쏘기 버튼 추가 / 어깨끈 많이 추가."
- **터치 조종 UI 신규 `src/ui/touchControls.ts`(leaf)**: 왼쪽 가상 조이스틱(이동, 아날로그) · 오른쪽 화면 드래그(시야 회전) · 오른쪽 아래 쏘기 버튼. 포인터 이벤트로 멀티터치 각각 추적. coarse 포인터(폰)일 때만 표시 → 데스크톱 무영향.
- **`pvpMode.ts` 배선**: 조이스틱 아날로그 벡터(moveVX/VY)를 이동에 반영(살짝 밀면 천천히), onLook 이 targetYaw/Pitch 조정(TOUCH_LOOK_SENSITIVITY 0.006), onFire 가 발사. 캔버스 마우스 핸들러는 `pointerType==='touch'` 를 무시해 터치 시야/발사와 충돌 방지(폰은 포인터락 없음). 매치 시작 시 표시, 종료/이탈 시 숨김.
- **어깨끈 4종 추가(총 5종)**: 스포티(네모 버클)·구슬(구슬 2알)·리본(도톰 밴드+리본 매듭)·별(별 참 대롱). 각자 전용 메시 분기. 카탈로그 76→80종(PARTS 60→64, strap 1→5).
- **성능 계약**: 완전 장착 메시 합(56) 여유가 0이라 신규 어깨끈은 밴드1+장식2 = **메시 ≤3** 상한 준수(테스트로 강제). pvpMode 청크가 커져 totalJs 번들 예산 700→720KB로 한 단계 상향(three 562·app 109 개별 예산은 그대로, 코드분할 pvp 청크가 총합에 별도로 잡히는 구조 반영).
- **검증**: 110테스트·typecheck·아키텍처(main 831/850)·핫패스·build(총합 701.7/720KB)·브라우저 smoke 통과. 터치 실감은 실폰 필요. 버전 17→18.
