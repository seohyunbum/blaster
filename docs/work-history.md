...
## 2026-07-24 — 보관함 블래스터 PVP AI 드론 팝 아레나

- **플레이 루프**: 상단 PVP 버튼 → 보관함 작품 선택 → 무표정 구형 AI 드론 3기와 순차 1대1. 매 라운드 양측 체력 10. 파워=팝 파워, 연사=발사 간격, 정확=퍼짐, 다루기=조준 추종·좌우 이동으로 실제 성능에 연결.
- **구조**: `PvpSession`(순수 규칙), `PvpArena`(64발 양방향 풀·스윕 충돌), `PvpMode`(입력·카메라·수명주기), `pvpHud`(보관함·체력·결과)로 분리. PVP는 지연 로딩하고 기존 저장 모양을 바꾸지 않아 `SAVE_VERSION=1` 유지.
- **안전**: 사람·동물 대신 눈·표정·신체 부위가 없는 구형 드론만 사용. 체력 0은 사망이 아니라 에너지 충전·다음 라운드·다시 도전으로 표현.
- **회귀 수정**: 빠른 `PVP→공방→PVP` 전환의 지연 로딩 중복 생성은 단일-flight로 차단. 대형 보관함 블래스터가 카메라 월드 좌표에 밀리던 뷰모델 맞춤을 로컬 좌표 기준으로 교정.
- **실패 기록**: Three.js UUID 생성이 전역 난수를 사용해 난수 테스트가 오탐 → 발사/update 코어만 검사하도록 경계 교정. 중단된 패치로 신규 테스트 파일이 NUL 손상 → 삭제·재생성 후 전체 NUL 스캔 통과. 앱 청크 129,067B로 120KB 초과 → PVP 동적 import 분리. 로컬 Playwright 누락 → 설치. 숨겨진 결과 오버레이가 로비 클릭을 가로챔 → `.pvp-hud [hidden]` 규칙 추가.
- **검증**: 단위 테스트 100개 + 아키텍처/핫패스 + 프로덕션 빌드/번들 + 실제 브라우저 PVP 로비→전투 smoke를 커밋 전에 실행. 배포 표식 11→12.
- **후속 작업**: WebAudio 발사 SFX의 핫패스 외부 할당 계측·재사용 검토, PVP 체력 10 상수의 UI/무대 중복 제거, 전체 JS 700KB 예산 여유(약 5KB) 확보 최적화.

## 2026-07-21 — PVP 아레나 조작 개선: 마우스 자유 시야 + WASD 이동 (사용자 요청)

10살 사용자 요청 "로블록스 아레나처럼 WASD로 움직이고 마우스로 화면 돌리며 적을 쏘기. 화면 잘 돌아가게. 총알 반영되게." (기존 협업자 PVP는 제자리 A/D 스트레이프 + 마우스 위치로 ±0.48 좁은 조준이라 시야가 안 돌아가는 느낌.)
- **pvpMode.ts 만 수정(모드 조정자, 저충돌)**:
  - **포인터락 자유 마우스룩**: 화면 클릭 시 requestPointerLock, movementX/Y 누적으로 yaw 무제한·pitch ±1.2 자유 회전. 잠금 전엔 화면 위치로 대략 조준(첫 클릭 유도). "화면 잘 돌아가게" 해결.
  - **WASD 자유 이동**: 기존 A/D 스트레이프 전용 → W/S(앞뒤)+A/D(좌우)를 보는 방향(yaw) 기준으로 이동, 아레나 경계(x±9, z −7..9) 클램프. 카메라 x·z 이동을 arena.update/fireRival 이 이미 playerPos로 받아 명중 판정 따라옴.
  - 조준을 즉시 반영(aimFollow 스무딩 제거)해 FPS 감각. Escape=onCollection 제거(포인터락 해제 충돌 방지, 보관함 버튼 유지).
- 발사는 기존 arena.firePlayer(카메라 전방) 재사용 — 자유 시야라 보는 방향으로 발사·명중("총알 반영"). pvpHud 안내문 갱신.
- **여러 적 로밍은 미구현(다음 단계)** — pvpArena(단일 rival) 구조 변경 필요, 협업자와 조율 대상.
- **검증**: 100테스트·typecheck 통과. 헤드리스 — PVP 진입·참전·프레임 진행 에러 0, AI 피격으로 플레이어 체력 감소 확인(양방향 발사 작동). 실제 마우스룩/WASD 감각은 실브라우저 필요. 버전 뱃지 12→13.

## 2026-07-21 — PVP 아레나 FPS: 여러 적 로밍 + 엄폐물 (사용자 요청)

10살 사용자 반복 요청 "WASD로 돌아다니고, 적도 여러 명 돌아다니고, 아레나 맵에서 지형지물 뒤에 숨으며 싸우기."
- **충돌 회피 설계**: 협업자의 `pvpArena`(1대1 드론태그)·`pvpSession`은 테스트(pvp-arena/pvp-session)가 참조 → 안 건드림. 신규 자체완결 모듈 `src/game/arenaField.ts` 를 만들고 `pvpMode.ts` 를 그걸 쓰게 재작성.
- **`arenaField.ts`**: 원형 아레나 + 고정 상자 엄폐물 6개(다트가 상자에 막힘=숨기 가능) + 로밍 적 드론 4기(wander·플레이어 조준·주기 발사, HP 3, 격파 후 2.2초 리스폰). 투사체 풀 64·sweepHitSphere 재사용. 드론 구체+총(사람·동물 형태 없음, 08 준수). resolvePlayer로 원-엄폐물/경계 충돌.
- **`pvpMode.ts` 재작성**: 로비는 pvpHud 재사용, 전투는 자체 HUD(조준점·상태바 "❤️/팝/적"·승패 카드). WASD 이동+포인터락 자유 시야+연사 발사. 15팝=승리, 체력0=패배·다시.
- **가드레일**: "처치"=금칙어 → "팝". 소스 한글 스캔 테스트가 잡음.
- **검증**: 100테스트·typecheck 통과. 헤드리스 — 진입·참전·프레임 에러 0, 적 4기 로밍·엄폐물 렌더·피격(체력10→9)·다트 차단 확인. 버전 13→14.

## 2026-07-25 — PVP 아레나 5스테이지 + 연승(win streak) (사용자 요청)

10살 사용자 요청 "5개 스테이지 클리어하면 만들기 화면으로 이동, 위쪽에 연승 표시(한 번 깨면 연승1)."
- **5스테이지 구조(pvpMode)**: 스테이지당 3팝(POPS_PER_STAGE), 5스테이지=15팝. 스테이지 클리어마다 체력 +2 보상 + 적 난이도↑(arenaField.setDifficulty = 1+(stage-1)*0.14: 적 속도·연사 배율). 상태바 "❤️/스테이지 N/5/팝 x/3/적 M".
- **클리어→만들기 이동 + 연승 +1**: 5스테이지 다 팝하면 `callbacks.onClearRun()` → main 이 streakBadge.increment() + setStation('workshop'). 패배(체력0)는 기존 결과카드(다시).
- **연승 뱃지(신규 `src/ui/streakBadge.ts`)**: 만들기 화면 위쪽 중앙 "🔥 연승 N". localStorage('blaster_win_streak') 저장(세이브 스키마 미변경, additive). setStation 이 workshop 일 때만 표시. main.ts 배선 최소(+8줄, 아키텍처 831/850 유지).
- **검증**: 100테스트·typecheck·아키텍처·핫패스·build·**브라우저 smoke** 로컬 통과(지난 배포 실패 재발 방지). 실브라우저 — 연승 뱃지 워크숍 표시/PVP 숨김·에러 0. 버전 14→15.
