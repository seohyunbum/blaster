...
- 단위 = 미터. 76종 기준 몸통+배럴 구조 길이는 기본형 약 **0.43~1.01m**, morph 극단 포함 **합성 전장 상한 = 1.30m**다. 몸통 max + 배럴 max ≤ 1.30은 09 봉투 전수 검사가 보장한다.
...
**2026-07-21 가상 토이 계열 오버레이** — `BODY_DIMS.detail`과 `BARREL_DIMS.style`로 카탈로그 고유 시그니처를 데이터에서 선택하고, 각 슬롯 빌더의 전용 분기로 나머지 형태를 만든다.

| 계열 | 몸통 시그니처 | 부착 파츠 시그니처 |
|---|---|---|
| 코멧 | 장축 캡슐 + 측면 구슬 2 | 구슬 고리 튜브·앞뒤 빛구슬 뷰어·별구슬 꼬리·3중 고리 팁 |
| 터빈 | 넓은 RoundedBox + 측면 휠 2 | 동력 휠 튜브·U자 그립·평행 지지 꼬리·3셀 박스·4방울 팁 |
| 버즈 | 짧은 구형 셸 + 범퍼 2 | 방울 튜브·빛방울 뷰어·손바닥 구형 그립·초소형 꼬리/팩 |
| 리버스 | 캡슐 + 후방 방울/고리 | 균형 구슬 튜브·브리지 뷰어·후방 방울 그립·측면 포드 |
| 듀오 | RoundedBox + 측면 캡슐 2 | 기본 2열 트윈 튜브·쌍둥이 팩·쌍둥이 안전 팁 |
| 후크 | 캡슐 + 측면 곡선 고리 | 종형 튜브·곡선 TubeGeometry 그립·접이 루프 꼬리 |
| 레이서 | 낮은 RoundedBox + 옆날개 2 | 지느러미 튜브/뷰어/그립/꼬리·방울 안전 팁 |

- 파츠당 메시 수 **≤14 (수치 정본 = 07 성능 예산 표, 2026-07-21 스코프 변경)** — verify 게이트 단위테스트로 강제: `countMeshes(buildPart(id, opts).group) <= 14` (§8). **morph 극단값에서도 재검사**하며, 장식 morph가 메시를 추가하므로 기본/0/0.5/1 및 단일 튜브+장식 최대 조합을 모두 돈다.
...
- **축 실수 기계 검출**: 몸통 구조 셸·배럴 구조 튜브의 실제 boundingBox가 렌더러와 공유하는 실측 정본과 **기본·극단 morph 모두 ±10%** 일치하는 테스트를 verify에 포함. t=0.5를 원시 카탈로그 치수 1.0으로 간주하지 않는다(비대칭 morph 범위와 충돌 방지). 봉투 상·하한은 09의 전수·속성 테스트가 소유.
...
1. 노출 모서리는 RoundedBox radius = 최소변의 **8~42%**(기본 25%). 순수 BoxGeometry 는 내부 은면에만. **morph 재생성 시 라운드 반경은 재계산**한다 — 스케일로 뭉개지 않는다 (09 §7).
2. 두께 과장 — 실물 비율 대비 단면적 **1.3~1.5배** (전작 몬스터 비주얼의 통통 비율 감각). morph 극단에서도 토이 프로포션이 유지되는 근거는 09 §7 봉투(장착부 최소 반경 ≥0.019m 등)의 설계 시점 사전 보정.
...
| 4 | 몸통 셸·배럴 튜브 실제 boundingBox — 공유 실측 정본 대비 **기본·극단 morph ±10%** | 회전축·스케일 실수 |
| 5 | Playwright: 동일 은색 몸통의 metal vs matte 렌더 정규 픽셀 차 > 0.1% | 환경맵 배선 유실(no-op 재발) |
...
| 합성 전장 쿼터 → **1.30m** | §1.1 반영 완료 (76종 재기준선 + morph 극단 전수 검사) |
