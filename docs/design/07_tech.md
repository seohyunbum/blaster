...
| --- | --- | --- |
| **main.ts = 지휘자** — 루프·입력·배선만, 신규 로직 금지 | AGENTS.md §1 "제1원칙" | `check-architecture.mjs`가 leaf 역참조와 `MAX_MAIN_LINES = 850`을 강제 |
| **leaf 규칙** — `game/`·`ui/` 는 main.ts import 금지 | AGENTS.md §2 의존 방향 | `scripts/check-architecture.mjs` **무수정 복사** (실물 확인: 경로 하드코딩 없음, 그대로 동작) |
| **크기 ratchet** — main은 850줄 이하 | AGENTS.md §1 | `check-architecture.mjs`에서 의존 방향과 함께 강제. 현재 786줄 |
| **핫패스 할당 금지** — update*/animate*/tick* 내 `new THREE.*` 차단 | AGENTS.md §4 | `check-hotpaths.mjs`가 실제 함수 블록을 검사 |
...
### 1.2 성능 예산 단일표 (실행 정본 = `src/game/budgets.ts`)

테스트와 런타임은 `PERFORMANCE_BUDGETS`를 직접 import한다. 이 표는 사람이 읽는 미러이며 값 변경 시 코드·테스트와 함께 수정한다.
...
| 가시 메시 (browser-smoke 실측) | ≤ 800 | `traverseVisible`로 공방·사격장 자동 대조 — 전작(~6,160) 대비 낮게 시작해 ratchet |
| 평균 프레임 (수동 step) | ≤ 8ms | 고정 60fps update+render 12회 평균을 Playwright에서 자동 대조 |

### 1.3 verify 체인

```json
"verify": "npm run typecheck && npm run test && npm run check:architecture && npm run check:hotpaths",
...
```
src/
├── main.ts                    # 부팅·입력·렌더 루프·UI 배선(850줄 ratchet)
├── game/
│   ├── definitions.ts         # SLOT/STATION/PROJECTILE 단일 레지스트리
│   ├── budgets.ts             # 성능 예산 정본
│   ├── types.ts               # discriminated PartDef·Blaster·ShotProfile
...
- **편집 상태의 단일 소유자는 `EditorSession`**: active 참조·undo·morph 제스처·랜덤·색칠 변경을 캡슐화한다. `main.ts`는 결과에 따라 재빌드·사운드·자동저장만 배선한다.
...
배포 흐름은 **push → GitHub Actions → GitHub Pages**. 현재 `vite.config.ts`는 상대 base와 Three vendor 청크를 사용한다:

```typescript
export default defineConfig({
  base: './',
  build: { rolldownOptions: { output: { codeSplitting: { groups: [/* three */] } } } },
});
```

Actions는 `verify:full`의 실제 Chromium smoke까지 통과한 dist만 Pages artifact로 올린다. 세이브는 출처(localhost/github.io)별 분리이므로 아들 PC에서는 처음부터 Pages URL로만 플레이하게 해 세이브 혼선을 차단한다. 플랫폼은 PC 데스크톱 우선(결정문 22 — 바탕화면 실행 아이콘).
...
| **M5 폴리시** | 아들 단독 플레이 가능 | 튜토리얼 코치마크·사운드 폴리시·**스티커**·극단 스크린샷 육안 승인(기본 visual/perf 자동화는 선행 완료) | 조립→변형→색칠→사격 전 루프 무도움 완주, `verify:full` 녹색 + 육안 승인 | 1~2 |
